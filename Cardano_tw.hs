...
scpToAllNodes node = do
  (exitcode, output) <- shellStrict (scpToNode node "receiver-logging.yaml" "/home/timewarp/receiver-logging.yaml") empty
  case exitcode of
    ExitSuccess -> return ()
    ExitFailure code -> echo $ "Scp to " <> node <> " failed with " <> (T.pack $ show code)
  (exitcode, output) <- shellStrict (scpToNode node "sender-logging.yaml" "/home/timewarp/sender-logging.yaml") empty
  case exitcode of
    ExitSuccess -> return ()
    ExitFailure code -> echo $ "Scp to " <> node <> " failed with " <> (T.pack $ show code)
  (exitcode, output) <- shellStrict (scpToNode node "peers_tw.txt" "/home/timewarp/peers.txt") empty
  case exitcode of
    ExitSuccess -> return ()
    ExitFailure code -> echo $ "Scp to " <> node <> " failed with " <> (T.pack $ show code)
