...
runexperiment = do
  ...
  shells (sshForEach "systemctl restart cardano-node") empty
  nodes <- getNodes
  sh $ using $ parallel nodes scpFromAllNodes
  -- TODO: note what parameters were used for experiment?
  shells (sshForEach "systemctl stop cardano-node") empty
  -- TODO: tar it up?
...
scpFromAllNodes node = do
  ...
  (exitcode, output) <- shellStrict (scpPerNode node "/var/lib/cardano-node/cardano-node.log" (workDir <> "/cardano-node-" <> node <> ".log")) empty
  case exitcode of
    ExitSuccess -> return ()
