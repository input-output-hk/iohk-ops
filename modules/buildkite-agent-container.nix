...
in with lib;
{
  ...
  config = let
    createBuildkiteContainer = { containerName
                               , baseVethIp ? "10.254.1"
                               , hostIpo4 ? "1"
                               , ipo4 ? "10"
                               , metadata ? "system=x86_64-linux,queue=test"
                               }: {
      name = containerName;
      value = {
        autoStart = true;
  ...
  in {
    ...
    containers = let
      buildkiteContainerList = [
        ...
        { containerName = "ci${cfg.hostIdSuffix}-3"; ipo4 = "12"; metadata = "system=x86_64-linux,queue=daedalusTest"; }
        { containerName = "ci${cfg.hostIdSuffix}-4"; ipo4 = "13"; metadata = "system=x86_64-linux,queue=daedalusTest"; }
      ];
    in
