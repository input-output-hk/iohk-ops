...
rec {
  ...
  cpu_monitor = {
    name = "High CPU usage";
    type = "metric alert";
    query = config: "avg(last_5m):avg:system.load.norm.1{env:${config.deployment.arguments.environment},depl:${config.deployment.name}} by {host} > 0.9";
  ...
  disk_monitor = {
    ...
    type = "metric alert";
    query = config: "max(last_5m):avg:system.disk.in_use{env:${config.deployment.arguments.environment},depl:${config.deployment.name}} by {host,device} > 0.9";
    monitorOptions.thresholds = {
      warning = "0.8";
      critical = "0.9";
    };
  ...
  cardano_node_simple_process_monitor = {
    ...
    type = "service check";
    query = config: "\"process.up\".over(\"env:${config.deployment.arguments.environment},depl:${config.deployment.name}\",\"process:cardano-node-simple\").by(\"host\",\"process\").last(5).count_by_status()";
    monitorOptions.thresholds = {
      warning = 2;
      critical = 4;
      ok = 2;
  ...
  cardano_explorer_process_monitor = {
    ...
    type = "service check";
    query = config: "\"process.up\".over(\"env:${config.deployment.arguments.environment},depl:${config.deployment.name}\",\"process:cardano-explorer\").by(\"host\",\"process\").last(5).count_by_status()";
    monitorOptions.thresholds = {
      warning = 2;
      critical = 4;
      ok = 2;
