...
let
  ...
  justIo = pkgs.mkShell {
    name = "io";
    buildInputs = with pkgs; [ iohk-ops (builtins.trace (builtins.attrNames iohkpkgs.cardano-node-pkgs.nix-tools.exes) iohkpkgs.cardano-node-pkgs.nix-tools.exes.cardano-node) terraform_0_11 iohkpkgs.nixops ];
    passthru = {
